...
1. Uploads a training and a validation dataset. The validation dataset is optional.
2. Creates a fine-tuning job.

   For more information about the fine-tuning job parameters, see the specification of the [fine-tuning job object](https://docs.nebius.com/studio/fine-tuning/how-to-fine-tune#job-specs).
3. Checks that the job status is `succeeded`.
...
4. Checks that the training has been successful. To do this, check the job events. They are created when the job status changes.
...
5. Retrieves the contents of the files with the fine-tuned model.
...
6. Saves the contents to files. The script creates a directory per checkpoint and saves the files into these directories.
